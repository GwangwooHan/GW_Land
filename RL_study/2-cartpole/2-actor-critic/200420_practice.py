...
# 정책신경망의 출력을 받아 확률적으로 action을 선택
    def get_action(self, state):
        policy, _ = self.model(state) # _ 만쓰는 것은 위의 return 값이 2개가 있는데 굳이 안받겠다라는 뜻인 것 같음
        policy = np.array(policy[0])
        return np.random.choice(self.action_size, 1, p = policy)[0]

    # 각 time step마다 정책신경망과 가치신경망을 업데이트
    def train_model(self, state, action, reward, next_state, done):
        model_params = self.model.trainable_variables
        with tf.GradientTape() as tape:
            policy, value = self.model(state)
            _, next_value = self.model(next_state)
            # Advantage: 아래 target으로 구함
            target = reward + (1-done) * self.discount_factor * next_value[0]

            # 정책 신경망 오류함수 구하기
            one_hot_action = tf.one_hot([action], self.action_size)
            action_prob = tf.reduce_sum(one_hot_action*policy, axis= 1)
            # 정책신경망 오류함수이기 떄문에 이 오류함수를 통해 가치신경망을 업데이트하지 않기 위해 stop._gradient 적용
            advantage = tf.stop_gradient(target - value[0])
            actor_loss = tf.math.log(action_prob+1e-5)*advantage
            actor_loss= -tf.reduce_mean(actor_loss)

            # 가치 신경망 오류함수 구하기
            # DQN과 마찬가지로 target 값은 tf.stop_gradient 적용하여 target이 변하지 않게 함
...
"""
1. 정책신경망을 통해 확률적으로 행동을 선택
2. 선택한 행동으로 환경에서 한 타임스텝 진행
3. 환경으로부터 s', r 수령
4. 샘플(s,a,r,s') 를 통해 시간차 에러를 구하고 어드밴티지 함수 구함
5. 시간차에러로 가치신경망을, 어드밴티지 함수로 정책신경망을 업데이트 
"""
