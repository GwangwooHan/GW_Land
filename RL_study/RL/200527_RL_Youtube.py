...
# Reward 가 -1 이므로 음수로 적용
q_table = np.random.uniform(low=-2, high=0, size = (DISCRETE_OS_SIZE + [env.action_space.n])) # (20, 20, 3) size
...
for episode in range(EPISODES):
    ...
    while not done:
        ...
        if not done:
            ...
            currnet_q = q_table[discrete_state + (action, )]
            new_q = (1-LEARNING_RATE)*currnet_q + LEARNING_RATE * \
