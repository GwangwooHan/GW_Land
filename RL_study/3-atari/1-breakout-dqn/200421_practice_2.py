...
class DQN(tf.keras.Model):
    def __init__(self, action_size, state_size):
        ...
        # 필터의 개수, 필터 크기, Conv 연산시 필터가 이동하는 폭: strides, 활성함수 지정
        self.conv1 = Conv2D(32, (8,8), stdrides = (4,4), activation = 'relu', input_shape = state_size)
...
class DQNAgent:
    def __init__(self, action_size, state_size = (84,84,4)):
        ...
        # replay memory, 최대 크기 100,000
        self.memory = deque(maxlen = 100000)
        # 게임 시작 후 랜덤하게 움직이지 않는 것에 대한 옵션
        # Agent가 초반에 일정 기간 동안 아무것도 하지 않는 구간을 무작위로 설정하며, 그 상한선을 no_op_steps로 설정 (이 코드에서는 30)
        self.no_op_steps = 30
    ...
    # Replay memory에서 무작위로 추출한 배치로 모델 학습
    def train_model(self):
        ...
        with tf.GradientTape() as tape:
            # 현재 history에 대한 모델의 큐함수
            predicts = self.model(history)
            one_hot_action = tf.one_hot(actions, self.action_size)
            predicts = tf.reduce_sum(one_hot_action*predicts, axis = 1)

            # 다음 history에 대한 타겟 모델의 큐함수
    ...
    # 학습속도를 높이기 위해 게임화면 전처리 (크기, 흑백)
    # 게임 화면 전처리 [210,160,3] -> [84,84,1]
    # 전처리 거친 이미지를 state로 설정
...
if __name__ == "__main__":
    ...
    for e in range(num_episode):
        ...
        # 프레임을 전처리한 후 4개의 상태를 쌓아서 입력값으로 사용 (첫 상태에서는 4개의 화면을 모을 수 없기 때문에 똑같은 화면을 복사하여 4개 이용)
        ...
        while not done:
            ...
            # 바로 전 history를 입력으로 받아 action 선택
            ...
            # 선택한 action으로 환경에서 한 타임스텝 진행
            observe, reward, done, info = env.step(real_action)
            # 각 타임 스텝마다 상태 전처리
            # history에서 오래된 state는 버리고 전처리를 거친 새로운 state를 채우는 과정
        ...
        if len(agent.memory) >= agent.train_start:
            agent.train_model()
            # 일정 시간마다 타겟 모델을 예측모델의 가중치로 업데이트
            ...
            # 1000 에피소드마다 모델 저장
            if e % 1000 == 0:
                agent.model.save_weights("./save_model/model2", save_formate="tf")
